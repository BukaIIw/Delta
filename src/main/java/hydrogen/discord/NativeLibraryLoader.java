...
import hydrogen.util.BooleanUtils;
...
public class NativeLibraryLoader {
    private static final String PROP_LOG = "hydrogen.loader.log";
    private static final String PROP_DIR = "hydrogen.loader.dir";
    private static final String PROP_PURGE = "hydrogen.loader.purge";
    ...
    static Path materialize(Source src, String resourceName, Path dir, Trace t) {
        ...
        try {
            ...
            if (!Files.isRegularFile(target, new LinkOption[0])) {
                IllegalStateException fail = new IllegalStateException("hydrogen: cannot extract native library '" + resourceName + "' into " + dir + t.tail());
                fail.initCause(e2);
                throw fail;
            }
    ...
    private static Source jarSource(String resource, String buildId, Trace t) {
        ...
        if (url == null) {
            throw new IllegalStateException("hydrogen: bundled native library '" + resource + "' is missing from the jar" + t.tail());
        }
        try {
...
public URL url() {
    ...
    private static void load(String path, Trace t) {
        try {
            ...
            t.flush();
            UnsatisfiedLinkError fail = new UnsatisfiedLinkError("hydrogen: failed to load native library " + path + hint(e) + t.tail());
            fail.initCause(e);
            throw fail;
        }
    }
    ...
    private static Path extractDir(Trace t) {
        ...
        t.log("dir=" + dir + (override == null ? "" : " [hydrogen.loader.dir]"));
        return dir;
    }
    ...
    static final class Trace {
        ...
        void flush() {
            ...
            for (String l : this.lines) {
                sb.append(stamp).append(" [hydrogen-loader] ").append(l).append(System.lineSeparator());
            }
            sb.append(stamp).append(" [hydrogen-loader] done in ").append((System.nanoTime() - this.startNanos) / 1000000).append(" ms").append(System.lineSeparator());
            this.lines.clear();
            if ("1".equals(where) || BooleanUtils.e.equalsIgnoreCase(where)) {
                ...
                try {
                    ...
                    System.err.println("[hydrogen-loader] cannot write " + where + ": " + e);
                    System.err.print(sb);
                }
            }
        }
